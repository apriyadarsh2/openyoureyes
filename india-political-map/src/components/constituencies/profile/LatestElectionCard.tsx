...
export default function LatestElectionCard({ election }: Props) {
  return (
    <section className="overflow-hidden rounded-3xl border border-politic-border bg-politic-card shadow-sm">
      ...
      {/* KPIs */}
      <div className="border-t border-politic-border bg-politic-inner/50 px-8 py-8">
...
function CandidateCard({ title, icon, color, name, party, votes }: CandidateProps) {
  const bg =
    color === "green"
  ...
  return (
    <div className={`rounded-2xl border p-6 ${bg}`}>
      ...
      <p className="mt-2 text-politic-muted font-medium">
        {party}
      </p>
      <div className="mt-6">
...
function StatCard({ icon, label, value }: StatProps) {
  return (
    <div className="rounded-2xl border border-politic-border bg-politic-card p-5">
