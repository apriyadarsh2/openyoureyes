    ...
    <div className="w-full flex flex-col gap-1.5">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-baseline gap-2">
            <span className="text-xl font-black text-[#F2F1EC] tracking-tight leading-none">{year}</span>
            <span
        ...
        <div className="flex gap-1.5">
          <button
            onClick={() => setYear(Math.max(1947, year - 1))}
            className="w-5 h-5 bg-[#14192b] border border-[#2d3654] rounded flex items-center justify-center text-[10px] text-[#F2F1EC] hover:bg-[#2d3654] transition-colors"
          >
            ◀
          </button>
          <button
      ...
      <div className="relative pt-0.5">
        <input
          ...
          value={year}
          onChange={(e) => setYear(Number(e.target.value))}
          className="w-full h-1.5 rounded-full cursor-pointer appearance-none bg-[#14192b]"
          style={{ background: `linear-gradient(to right, ${currentEra.color} ${pct}%, #14192b ${pct}%)` }}
        />
      </div>
    </div>
  );
