...
export default function CriminalCasesPage() {
  ...
  if (!cases.length) {
    return (
      <div className="rounded-xl border border-white/10 bg-[#1F2937] p-10 text-center text-gray-400 font-medium shadow-sm">
        No criminal cases available.
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-6 lg:grid-cols-2 mt-6">
        <CriminalSeverityChart cases={cases} />
        <CriminalStatusChart cases={cases} />
      </div>
      ...
      <div className="space-y-6">
        {cases
          .sort((a, b) => b.year_filed - a.year_filed)
          .map((item) => (
            <CriminalCaseCard
              key={item.case_id_source}
              criminalCase={item}
